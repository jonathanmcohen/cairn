...
export default async function SecuritySettingsPage() {
  ...
  // v0.10.2 S11 — the SessionsCard sign-out confirm names the account. The JWT
  // session only carries the user id (see auth/config.ts session callback), so
  // read the email from the users record by id (the account-profile pattern).
  const [signedInUser] = await getDb()
  ...
  return (
    <main className="mx-auto max-w-2xl space-y-6 p-6">
      ...
      <PasskeysCard />
      <SessionsCard userEmail={signedInUser?.email} />
