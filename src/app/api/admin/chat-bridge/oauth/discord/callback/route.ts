...
export async function GET(req: Request): Promise<Response> {
  try {
    ...
    try {
      payload = await verifyOauthState(state, 'discord');
    } catch {
    ...
    const clientId = process.env.CAIRN_DISCORD_CLIENT_ID;
    const clientSecret = process.env.CAIRN_DISCORD_CLIENT_SECRET;
    if (!clientId || !clientSecret) {
      return NextResponse.json({ error: 'Discord OAuth not configured' }, { status: 500 });
    }
    const origin = await publicOrigin();
    const redirectUri = await oauthRedirectUri(origin, 'discord');
    const result = await exchangeCode('discord', { code, clientId, clientSecret, redirectUri });
    ...
    return NextResponse.redirect(`${origin}/admin/chat-bridge?installed=discord`);
  } catch (err) {
