...
- [ ] **Step 1: Write failing test `tests/lib/mcp/error.test.ts`**
...
- [ ] **Step 3: Write `src/lib/mcp/error.ts`**
...
- [ ] **Step 1: Write failing test `tests/lib/mcp/tools.test.ts`**
...
- [ ] **Step 1: Write failing test `tests/lib/mcp/dispatcher.test.ts`**
...
- [ ] **Step 3: Write `src/lib/mcp/dispatcher.ts`**
...
- [ ] **Spec coverage:** Section 2.2 (allowlist + presets) + Section 3 G2 (tool registry shape, enforcement order, initial 20-tool list minus `search.semantic`) + Section 5.4 (`mcp_tool_called_total{tool, outcome}` outcomes vocabulary, noted but wired in P9). ✓
- [ ] **Enforcement order:** existence → scope → allowlist → rate-limit → schema → handler. The dispatcher does NOT re-implement `requirePageAcl` — the embedded test asserts the dispatcher source file never imports it. ✓
- [ ] **Logging:** every outcome (success OR error) writes one `token_usage_log` row with `route=mcp:<toolId>` + `mcp_tool=<toolId>` + the integer-status mapped from the McpError code. ✓
- [ ] **No new business logic:** all 19 handlers wrap existing v0.5/v0.6 library helpers via dynamic `import('@/lib/...')`. The one allowed-deviation exception is a thin `getWorkspaceInfo` helper if one doesn't already exist. ✓
- [ ] **Placeholder scan:** no `TODO` / `FIXME` / `TBD` markers; all code blocks are complete. ✓
