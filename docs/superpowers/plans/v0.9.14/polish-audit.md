...
| 8 | Page-cover hairline | **SHIP-ALREADY** | Already implemented in CSS at `globals.css:383` — `.cairn-cover { … border-bottom: 1px solid hsl(var(--border)); }` (v0.9.11 #8). All four cover variants carry `cairn-cover`, so the token-driven hairline already renders (light/dark aware). The earlier audit read only the className (no `border-b`) and missed the CSS rule inside the `globals.css:379-386` block. | None — adding `border-b` in the className would duplicate the existing CSS `border-bottom`. Task dropped from Plan U. |
