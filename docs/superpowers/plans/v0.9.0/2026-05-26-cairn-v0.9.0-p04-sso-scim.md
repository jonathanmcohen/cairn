...
- [x] **Step 1: Confirm P1 + P2 + P3 surfaces**
...
- [x] **Step 2: Confirm `workspaces` role-enum shape**
...
- [x] **Step 3: Confirm spec §2 G1 #2 does NOT require an arbitrary-name `scim_groups` table**
...
- [x] **Step 4: Confirm `requireRole` + `recordAudit` are in their P1/P2 locations**
...
- [x] **Step 1: Write failing test `tests/lib/sso/scim-token.test.ts`**
...
- [x] **Step 3: Implement `src/lib/sso/scim-token.ts`**
...
- [x] **Step 1: Write failing test `tests/lib/sso/scim-filter.test.ts`**
...
- [x] **Step 3: Implement `src/lib/sso/scim.ts`**
...
- [x] **Step 1: Write failing test `tests/api/scim/users.test.ts`**
...
- [x] **Step 3: Implement `src/app/api/scim/v2/Users/route.ts`**
...
- [x] **Step 4: Implement `src/app/api/scim/v2/Users/[id]/route.ts`**
...
- [x] **Step 1: Write failing test `tests/api/scim/groups.test.ts`**
...
- [x] **Step 3: Implement `src/app/api/scim/v2/Groups/route.ts`**
...
- [x] **Step 4: Implement `src/app/api/scim/v2/Groups/[id]/route.ts`**
...
- [x] **Step 1: Write failing test `tests/api/admin/sso/scim-tokens.test.ts`**
...
- [x] **Step 3: Implement `src/app/api/admin/sso/scim-tokens/route.ts`**
...
- [x] **Step 4: Implement `src/app/api/admin/sso/scim-tokens/[tokenId]/route.ts`**
...
- [x] **Step 1: Implement `src/app/api/admin/sso/route.ts` (combined IdP list)**
...
- [x] **Step 2: Implement `src/components/admin/sso/scim-token-list.tsx`**
...
- [x] **Step 3: Implement `src/app/(app)/admin/sso/page.tsx`**
...
- [x] **Step 5: A11y sanity (UI surfaces)**
...
- [x] **Step 6: Mark this plan's checkboxes and roadmap, commit**
...
- [x] **Spec coverage:** §2 G1 #2 (SCIM 2.0 /Users + /Groups + admin UI), §5 "JIT user provisioning explicitly skipped — SCIM is the provisioning surface" (POST creates users + memberships; new email DOES create a user — that's the SCIM contract, not JIT). The complex-filter deferral is enforced (filter parser only accepts the two documented operators). ✓
- [x] **Groups-as-roles decision (Task 1 Step 3):** documented at the top of this plan. Defers arbitrary `scim_groups` to v1.0. ✓
- [x] **Token security:** raw token returned in the mint response ONCE; DB stores only SHA-256 hash. `cairn_scim_` prefix mirrors the v0.7 PAT prefix convention; gitleaks already redacts the `cairn_pat_` prefix — add a redaction rule for `cairn_scim_` if not already covered (cross-plan concern; out of scope for this plan's commit but should be queued for P44 prep). ✓
- [x] **Constant-time compare:** `crypto.timingSafeEqual` on the buffered hashes belt-and-suspenders. ✓
- [x] **Filter parser:** rejects logical combinators, parens, every unsupported operator, malformed syntax. Returns SCIM `400 invalidFilter` (not generic 400) — IdPs interpret the scimType field. ✓
- [x] **Cross-workspace 404:** every admin-CRUD route uses the workspace-scoped query helper and returns 404 (never 403) on cross-workspace ids. ✓
- [x] **Audit coverage:** `sso.scim.token.minted` + `sso.scim.token.revoked` emit at mint + revoke (Task 6); `sso.idp.created/updated/deleted` from P2/P3 already cover IdP CRUD. The dashboard reads the last 50 entries filtered to all five actions. ✓
- [x] **Type consistency:** `ScimUser`, `ScimGroup`, `ScimTokenRow`, `ParsedScimFilter`, `VerifiedScimToken` — named and exported where used. ✓
- [x] **Placeholder scan:** no TODO/TBD/FIXME. Every step has full code. ✓
- [x] **Build coverage:** `pnpm build` required (new client component + new pages). Step 4 of Task 7 runs it. ✓
- [x] **A11y:** queued into P44 sweep per plans-index. Touch-target audits for `/admin/sso` happen there. ✓
- [x] **Retrospective rules:** §1 release branch, §2 plan-review pass (Task 1 verifies workspace_members role enum + spec scim_groups absence + audit-actions presence before implementer dispatch), §3 verify-before-commit, §4 no time-sleep flakes, §7 coupling-with-coordination (P2/P3 admin links resolve to the same `/admin/sso` dashboard surface — Task 7 Step 3), §9 checkbox flip, §10 roadmap update. ✓
